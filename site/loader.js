...
var deviceOrientationCamera = document.getElementById(
...
var renderFunction = function () {
  ...
  if (scene) {
    if (!sceneChecked) {
      ...
      engine.loadingUIText =
        "Streaming items..." + (remaining ? remaining + " remaining" : "");
...
document
  .getElementById("cameraClickableTag")
...
touchCamera.addEventListener("click", function () {
  ...
  var camera = new BABYLON.TouchCamera(
    "touchCamera",
...
anaglyphCamera.addEventListener("click", function () {
  ...
  var camera = new BABYLON.AnaglyphFreeCamera(
    "anaglyphCamera",
    scene.activeCamera.position,
    0.2,
    scene
...
toggleBandW.addEventListener("click", function () {
  if (scene && scene.activeCamera) {
    if (scene.activeCamera.__bandw_cookie) {
      ...
      scene.activeCamera.__bandw_cookie = new BABYLON.BlackAndWhitePostProcess(
        "bandw",
        1.0,
        scene.activeCamera
      );
...
toggleFxaa.addEventListener("click", function () {
  if (scene && scene.activeCamera) {
    if (scene.activeCamera.__fxaa_cookie) {
      ...
      scene.activeCamera.__fxaa_cookie = new BABYLON.FxaaPostProcess(
        "fxaa",
        1.0,
        scene.activeCamera
      );
...
toggleSepia.addEventListener("click", function () {
  if (scene && scene.activeCamera) {
    if (scene.activeCamera.__sepia_cookie) {
      ...
      scene.activeCamera.__sepia_cookie = new BABYLON.FilterPostProcess(
        "Sepia",
        sepiaKernelMatrix,
        1.0,
        scene.activeCamera
